...
//     try {
//       const res = await ragService.chat(question);
...
import React, { useState, useRef, useEffect } from 'react';
import { useSelector } from 'react-redux'; // 1. Import hook của Redux
import { ragService } from '../services/rag.service';
import Button from '../components/common/Button';
import Input from '../components/common/Input';
import Layout from '../components/layout/Layout';
// Import RootState để lấy kiểu dữ liệu (đường dẫn có thể khác tùy dự án của bạn)
...
const AIChatPage = () => {
  ...
  const handleUpload = async () => {
    ...
    // 3. Kiểm tra xem đã đăng nhập chưa
    if (!user || !user.id) {
    ...
    setUploading(true);
    try {
      // 4. Truyền user.id vào hàm upload
  ...
  const handleSend = async () => {
    ...
    // 5. Kiểm tra đăng nhập trước khi chat
    if (!user || !user.id) {
  ...
  return (
    <Layout>
        {/* ... (Phần giao diện giữ nguyên không cần sửa) ... */}
        <div className="max-w-6xl mx-auto h-[calc(100vh-100px)] flex flex-col gap-6">
